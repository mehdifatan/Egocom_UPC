...
class MyEnsemble(nn.Module):
    ...
    def forward(self, src1, src2, src3, src_att1, src_att2, src_att3, target_input, trg_att):
        pred1 = self.sfmx(self.model1(src1, target_input, src_att1, trg_att))
        pred2 = self.sfmx(self.model2(src2, target_input, src_att2, trg_att))
        pred3 = self.sfmx(self.model3(src3, target_input, src_att3, trg_att))
...
def main():
    ...
    if include_prior == True:
        ...
        model1 = individual_TF.IndividualTF(N1 + 3, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
        model2 = individual_TF.IndividualTF(N2 + 3, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
        model3 = individual_TF.IndividualTF(N3 + 3, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
    else:
        model1 = individual_TF.IndividualTF(N1, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
        model2 = individual_TF.IndividualTF(N2, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
        model3 = individual_TF.IndividualTF(N3, 4, 4, N=layers, d_model=emb_size, d_ff=2048, h=heads,
                                            dropout=dropout).float().to(device)
