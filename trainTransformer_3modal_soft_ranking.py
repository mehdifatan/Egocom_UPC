...
def train_epoch(model1, model2, model3, x_train, y_train, optim1, optim2, optim3, device):
    model1.train()
    model2.train()
    model3.train()
...
def eval_epoch(model1, model2, model3, x_val, y_val, device):
    model1.eval()
    model2.eval()
    model3.eval()
...
for epoch in range(max_epoch):
    train_loss, train_accuracy = train_epoch(model1,model2,model3, x_tr_dl, y_tr_dl,optim1,optim2,optim3,device)
    loss_values.append(train_loss)
    train_acc_values.append(train_accuracy)
    print('Epoch: %03i/%03i | Train Loss: %.3f | Accuracy: %.3f'%(epoch+1, max_epoch, train_loss ,train_accuracy))
    val_loss, val_accuracy = eval_epoch(model1,model2,model3, x_val_dl, y_val_dl, device)
    ...
    test_loss,test_accuracy = eval_epoch(model1,model2,model3, x_test_dl, y_test_dl, device)
