...
def train_epoch(model, x_train, y_train, optim, device):
    model.train()
...
def eval_epoch(model, x_val, y_val, device, optim):
    model.eval()
    epoch_loss, n_correct, n_total = 0, 0, 0
    inp = iter(x_val)
    with torch.no_grad():
...
def testing_epoch(model, x_test, y_test, device):
    model.eval()
    n_correct, n_total = 0, 0
    inp = iter(x_test)
    with torch.no_grad():
        for id_b, batch in enumerate(y_test):
            src = next(inp).to(device)
            # Target starting point
            start_of_sequence = torch.tensor(np.zeros((1, 4))).repeat(batch.shape[0], 1, 1)
            trg = torch.cat((start_of_sequence, batch), 1)
    ...
    for include_prior in include_prior_list:
       # del model
        ...
        if not include_prior and modal=='text_video_voxaudio':
            ...
            '''for id_s,setting in enumerate(settings):
                lr = setting['lr']
                dropout = setting['dropout']
                wd = setting['weight_decay']
                model=individual_TF.IndividualTF(N, 4, 4, N=layers,                                     
                                             d_model=emb_size, d_ff=2048, h=heads, dropout=dropout).float().to(device)
                optim = torch.optim.AdamW(model.parameters(), lr, betas = (0.9 , 0.98), eps=1e-9, weight_decay=wd)
            ...
            for epoch in range(max_epoch): 
                train_loss, train_accuracy = train_epoch(model, x_tr_dl, y_tr_dl,optim,device)
                loss_values.append(train_loss)
                train_acc_values.append(train_accuracy)
                print('Epoch: %03i/%03i | Train Loss: %.3f | Accuracy: %.3f'%(epoch+1, max_epoch, train_loss ,train_accuracy))
                val_loss, val_accuracy = eval_epoch(model, x_val_dl, y_val_dl, device, optim)
                ...
                test_accuracy = testing_epoch(model, x_test_dl, y_test_dl, device)
