...
def train_epoch(model1, model2, x_train, y_train, optim1, optim2, device):
    model1.train()
    model2.train()
    ...
    for id_b, batch in enumerate(y_train):
        ...
        # _, predicted1 = pred.max(1)
        # _, predicted2 = pred2.max(1)

        # loss

        # print("Target before first prediction")
        # print(np.shape(target))
        # print("prediction before first prediction")
        # print(np.shape(predicted1))
        loss1 = criterion1(pred1, target)
        loss1.backward()
        optim1.step()

        # print("target after first prediction")
        # print(np.shape(target))
        # print("prediction2 after first prediction")
        # print(np.shape(predicted2))
...
def eval_epoch(model1, model2, x_val, y_val, device):
    model1.eval()
    model2.eval()
...
parser.add_argument('--modals', default='video_voxaudio', type=str,
...
assert modals in ['text_voxaudio', 'text_video', 'video_voxaudio']

#--------------------------------
#modals = ['text_voxaudio', 'text_video', 'video_voxaudio']
...
if modals=='text_voxaudio':
    N1=899
    N2=1536
elif modals=='text_video':
    N1 = 899
    N2 = 6144
else:
    N1 = 6143
...
for epoch in range(max_epoch):
    train_loss, train_accuracy = train_epoch(model1,model2, x_tr_dl, y_tr_dl,optim1,optim2,device)
    loss_values.append(train_loss)
    train_acc_values.append(train_accuracy)
    print('Epoch: %03i/%03i | Train Loss: %.3f | Accuracy: %.3f'%(epoch+1, max_epoch, train_loss ,train_accuracy))
    val_loss, val_accuracy = eval_epoch(model1,model2,x_val_dl, y_val_dl, device)
    ...
    test_loss,test_accuracy = eval_epoch(model1,model2, x_test_dl, y_test_dl, device)
